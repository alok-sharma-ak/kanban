...
@Module({
  imports: [
    ConfigModule.forRoot({ isGlobal: true, envFilePath: ['packages/kanban-api/.env'], validationSchema: Joi.object({
      ...
      DATABASE_URL: Joi.string().required(), JWT_SECRET: Joi.string().min(32).required(), JWT_EXPIRES_IN: Joi.string().default('1d'), REDIS_URL: Joi.string().required(), CACHE_TTL_SECONDS: Joi.number().default(60),
    ...
    InfrastructureModule, AuthModule, UsersModule, BoardsModule, ColumnsModule, TasksModule, AttachmentsModule, HealthModule,
  ],
